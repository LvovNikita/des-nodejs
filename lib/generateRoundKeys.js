...
generateRoundKeys = function (key) {
    ...
    // FIXME: длина раундовых ключей получилась больше на 1 байт, чем нужно
    roundKeys = roundKeys.map(key => binaryToBuffer(key))
    // PC2: 56-bit key -> 48-bit key
    const result = roundKeys
        .map(key => permutate(key, _PC2)) // Array<Array<number> 48-bit>
